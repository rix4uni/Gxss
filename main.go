...
func main() {
	...
	flag.BoolVar(&Reflections, "hr", false, "Hide reflection details")
...
func checkreflection(link string) {
	...
	originalURL := u.String() // Save the original URL
	...
	for key, value := range q {
		...
		if match != nil {
			reflectedParams = append(reflectedParams, fmt.Sprintf("%s=%s", key, payload))
		}
		q.Set(key, tm)
	}
	
	if !Reflections && len(reflectedParams) > 0 {
		if verbose {
			fmt.Printf("Url : %q\n", originalURL)
			fmt.Printf("Reflected Params : %q\n", strings.Join(reflectedParams, ", "))
		} else {
			fmt.Printf("%s Reflected:[%s]\n", originalURL, strings.Join(reflectedParams, ", "))
		}
		if outputFile != "" {
			...
			if _, err := f.WriteString(fmt.Sprintf("%s Reflected:[%s]\n", originalURL, strings.Join(reflectedParams, ", "))); err != nil {
				log.Fatal(err)
			}
			f.Close()
		}
	}else if Reflections && len(reflectedParams) > 0 {
		if verbose {
			fmt.Printf("Url : %q\n", originalURL)
			fmt.Printf("Reflected Params : %q\n", strings.Join(reflectedParams, ", "))
		} else {
